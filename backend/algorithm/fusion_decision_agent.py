...
多源识别结果（车牌、交通手势、车主手势）的统一融合与决策输出。
核心能力：
1. 多源结果聚合：将同一时间窗口内的多个识别结果合并为统一摘要
2. 冲突消解：当多路识别结果冲突时，按优先级和置信度仲裁
3. 综合摘要：生成面向用户的可读决策建议（如"直行+车牌京A12345"）
4. 性能监控：记录每次融合的延迟与准确率指标

数据流：
    车牌识别结果 ──┐
    交通手势结果 ──┼──→ FusionDecisionAgent ──→ 融合摘要 + 决策建议
    车主手势结果 ──┘
"""
