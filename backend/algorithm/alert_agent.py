...
2. 告警分级：5级告警体系（critical/high/medium/low/info）
3. 频发升级：同一事件短时间内多次触发自动提升告警级别
4. LLM 摘要：可选的大模型告警摘要生成
5. SSE 推送：通过 Server-Sent Events 向前端实时推送告警

设计原则：
- 规则优先：先匹配确定性规则，再使用 LLM 兜底
- 低延迟：同步规则检测 < 5ms，异步 LLM 调用不阻塞主流程
- 可配置：告警阈值、LLM 开关均可动态调整
"""
