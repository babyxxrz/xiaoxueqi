...
输入：
- extract_hand_features.py 生成的 .npz 文件

输出：
- backend/models/owner_gesture/owner_gesture_classifier.pkl
- backend/models/owner_gesture/metrics.json
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train owner gesture classifier from hand features.")
    parser.add_argument(
        "--features",
        type=Path,
    ...
    parser.add_argument(
        "--model-output",
        type=Path,
        default=Path("backend/models/owner_gesture/owner_gesture_classifier.pkl"),
        help="模型输出路径。",
    )
    parser.add_argument(
        "--metrics-output",
        type=Path,
        default=Path("backend/models/owner_gesture/metrics.json"),
        help="评估指标输出路径。",
    )
